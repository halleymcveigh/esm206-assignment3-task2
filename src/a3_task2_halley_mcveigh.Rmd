...
**Note: You should get a warning about one piece dropped. What happens to the name in Row 49, which contains two dashes? The name following the second dash is dropped.
...
sw_skin_sep <- starwars %>% # Start from starwars data
  separate(col = skin_color, # Separate the 'skin_color' column
           ...
           sep = ",", # Separating at the first comma
           extra = "merge", # If additional spaces, merger all into second column
           remove = FALSE) # And *do not* remove the orginal column
...
some_dates <- c(12311992, 07162007, 11012020) # Create a vector of dates without spaces
...
gasol_hookshots <- lakers %>% # Start with lakers data
  filter(player == "Pau Gasol", type =="hook") %>% # Make new dataset for player 'Pau Gasol' and only his 'hook shots'
  mutate(date = ymd(date)) %>%  # Update date column to "Date" class
  mutate(month = month(date)) # Add a new column with 'month'

hook_shots_by_month <- gasol_hookshots %>% # Start with 'gasol_hookshots'
  count(month) # Number of hook shots per month # Add a new column for number of shots per month

hook_shots_by_month # View Gasol's hook shots per month
```
...
storms_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column danger
    status == "hurricane" ~ "high", # "high" danger for all hurricane status
    status = TRUE ~ "moderate")) # "moderate" danger for all other statuses
